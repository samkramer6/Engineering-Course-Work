...
[dist] = distance(lat, lon, lastLon, lastLat);
dista = dist/1000;
...
add1 = 0;
add2 = 0;
while z < numel(instVel) %calculates the sum of the instant velocity matrix
    add1 = (instVel(z) + add1);
    z=z+1;
end
    mean1 = (add1/numel(instVel));%finds the mean
    stdB = instVel - mean1;%subtracts the mean from the instant velocity matrix
    stdC = stdB.^2;%squares the difference matrix
v = 1;
while v <= numel(instVel)
    add2 = (stdC(v) + add2);%find the sum of the difference matrix
    v = v+1;
end
mean2 = (add2/numel(instVel));%finds the mean of the difference matrix
stdDev = sqrt(mean2);%squareroots the matrix to find the standard deviation
